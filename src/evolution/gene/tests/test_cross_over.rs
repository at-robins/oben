...
fn test_vec_cross_over() {
    // Test vectors of the same length.
    {
        ...
        let vec_a: Vec<BinarySubstrate> = vec![BitBox::from_boxed_slice(Box::new(a))];
        let vec_b: Vec<BinarySubstrate> = vec![BitBox::from_boxed_slice(Box::new(b))];
        let recombined = vec_a.cross_over(&vec_b);
        for i in 0..vec_a[0].len() {
            assert!(
                recombined[0].get(i) == vec_a[0].get(i) || recombined[0].get(i) == vec_b[0].get(i)
            );
        }
    }
    // Test vectors of different length.
    {
        ...
        let vec_a: Vec<BinarySubstrate> = vec![BitBox::from_boxed_slice(Box::new(a))];
        let vec_b: Vec<BinarySubstrate> = vec![
            BitBox::from_boxed_slice(Box::new(b)),
            BitBox::from_boxed_slice(Box::new(c)),
        ];
        let recombined = vec_a.cross_over(&vec_b);
        for i in 0..vec_a[0].len() {
            assert!(
