use super::*;
use crate::abnn::parameters::ConfigurableParametersBuilder;

fn config() -> Arc<ConfigurableParameters> {
    Arc::new(
        ConfigurableParametersBuilder::default()
            .neuron_value_halflife(16.0.into())
            .neuron_value_halflife_refractory(16.0.into())
            .build()
            .unwrap(),
    )
}
...
fn test_new() {
    let neuron = Neuron::new(Arc::clone(&config()));
    assert_ulps_eq!(neuron.value(), config().neuron_base_value());
...
fn test_update_value() {
    {
        ...
        let neuron = Neuron::new(Arc::clone(&config()));
        assert_eq!(neuron.value(), config().neuron_base_value());
    ...
    }
    {
        ...
        neuron.update_value(time_at_point(config().neuron_value_halflife() as usize));
        assert_ulps_eq!(
            neuron.value(),
            ((current_value - config().neuron_base_value()) / 2.0) + config().neuron_base_value()
        );
        neuron.update_value(time_at_point((2.0 * config().neuron_value_halflife()) as usize));
        assert_ulps_eq!(
            neuron.value(),
            ((current_value - config().neuron_base_value()) / 4.0) + config().neuron_base_value()
        );
    }
    {
